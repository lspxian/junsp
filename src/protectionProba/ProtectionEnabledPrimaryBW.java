...
public class ProtectionEnabledPrimaryBW extends AbstractLinkMapping {

	private String localPath ;
	public ProtectionEnabledPrimaryBW(SubstrateNetwork sNet) {
		super(sNet);
	...
	public void generateFile(VirtualNetwork vNet,Map<VirtualNode, SubstrateNode> nodeMapping) throws IOException{
		...
		Set<String> potential= availablePotential(vNet);
		...
		for (VirtualLink tmpl:vNet.getEdges()) {
			...
			for (SubstrateLink tmpsl:sNet.getEdges()){
				...
				String str="vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+ssnode.getId()+"sd"+dsnode.getId();
				if(potential.contains(str)){
					//objective
			...
			for(SubstrateNode snode:sNet.getVertices()){
				...
				for(SubstrateNode tmmpsn:nextHop){
					if(potential.contains("vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+snode.getId()+"sd"+tmmpsn.getId())){
		...
		for (SubstrateLink tmpsl:sNet.getEdges()){
			...
			for (VirtualLink tmpl:vNet.getEdges()) {
				srcVnode = vNet.getEndpoints(tmpl).getFirst();
				dstVnode = vNet.getEndpoints(tmpl).getSecond();
				if(potential.contains("vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+ssnode.getId()+"sd"+dsnode.getId())){
					BandwidthDemand bwDem=tmpl.getBandwidthDemand();
					//capacity constraint
	...
	public Set<String> availablePotential(VirtualNetwork vn){
		Set<String> result=new HashSet<String>();
		for(SubstrateLink tmpsl:sNet.getEdges()){
			SubstrateNode ssnode = sNet.getEndpoints(tmpsl).getFirst();
			SubstrateNode dsnode = sNet.getEndpoints(tmpsl).getSecond();
			for (VirtualLink tmpl:vn.getEdges()) {
				VirtualNode srcVnode = vn.getEndpoints(tmpl).getFirst();
				VirtualNode dstVnode = vn.getEndpoints(tmpl).getSecond();
				BandwidthDemand bwDem=tmpl.getBandwidthDemand();
				String str="vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+ssnode.getId()+"sd"+dsnode.getId();
				
				for(List<SubstrateLink> tempPath:tmpsl.getKsp()){
					double tmp=sNet.backupBottleneckCapacity(tempPath, tmpsl);
					if(tmp>=bwDem.getDemandedBandwidth()){
						result.add(str);
						result.add("vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+dsnode.getId()+"sd"+ssnode.getId());
						break;
					}
				}
