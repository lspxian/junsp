 ...
 * 
 * @author li
 * all information, but intra virtual link can't use inter substrate link
 */
public class MDasOD2 extends AbstractMultiDomainLinkMapping {
	Domain sNet;
	
	public MDasOD2(List<Domain> domains) {
		super(domains);
...
//		System.out.println(solution);
	...
	public void generateFile(VirtualNetwork vNet,Map<VirtualNode, SubstrateNode> nodeMapping) throws IOException{
		this.localPath="tmp/MDasOD2-"+vNet.getId()+".lp";
		...
		for (Iterator<VirtualLink> links = vNet.getEdges().iterator(); links.hasNext();) {
			...
			for (Iterator<SubstrateLink> slink = sNet.getEdges().iterator();slink.hasNext();){
				...
				//bounds
				if(srcVnode.getDomain().equals(dstVnode.getDomain())&&(tmpsl instanceof InterLink)){
					bounds = bounds + "0 = vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+ssnode.getId()+"sd"+dsnode.getId()+"\n";
					bounds = bounds + "0 = vs"+srcVnode.getId()+"vd"+dstVnode.getId()+"ss"+dsnode.getId()+"sd"+ssnode.getId()+"\n";
				}else{
		...
		obj = obj+ "\n";
		BufferedWriter writer = new BufferedWriter(new FileWriter(localPath)); //TODO
